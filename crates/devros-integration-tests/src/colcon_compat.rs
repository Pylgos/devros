...
fn should_exclude(path: &str) -> bool {
    ...
    if path.ends_with("/package.bash") || path.ends_with("/package.sh") || path.ends_with("/package.zsh") {
        return true;
    }
    ...
    if path.contains("/lib/") && !path.contains("python") && !path.ends_with(".so") && !path.contains("/site-packages/") {
        // Check if it's an executable (not a library or Python file)
        if !path.ends_with(".cmake") && !path.ends_with(".txt") && !path.contains(".") {
            return true;
        }
    }
                ...
                if path.as_str().ends_with(".dsv") {
                    ...
                    match (colcon_content, devros_content) {
                        (Ok(colcon_str), Ok(devros_str)) => {
                            ...
                            match (DsvEntries::parse_str(&colcon_str), DsvEntries::parse_str(&devros_str)) {
                ...
                if !colcon.is_dir {
                    ...
                    match (colcon_content, devros_content) {
                        ...
                        (Ok(_), Ok(_)) => {
                            errors.push(ComparisonError::ContentMismatch {
                                path: path.to_string(),
                                details: "File content differs".to_string(),
                            });
                        }
                        (Err(e), _) | (_, Err(e)) => {
                            errors.push(ComparisonError::ContentMismatch {
...
"#;
        ...
        assert!(entries.source_entries.contains("source;share/pkg/hook/test.dsv"));
        assert!(entries.source_entries.contains("source;share/pkg/hook/test.sh"));
        assert!(!entries.source_entries.iter().any(|e| e.ends_with(".ps1")));
        
        assert!(entries.other_entries.contains("prepend-non-duplicate;PATH;bin"));
    }
    ...
    fn test_should_exclude() {
        ...
        assert!(should_exclude("COLCON_IGNORE"));
