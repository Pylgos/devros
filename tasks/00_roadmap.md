...
- [x] **Materialization (実体化)**:
    - [x] `install` ディレクトリのマージ機能
    - [x] シンボリックリンクの解決（実ファイルコピー）
    - [x] Pythonパッケージのフリーズ（`.egg-link` 解消）
    - [x] *Test (Integration)*: シンボリックリンクが実ファイルに置換されているか、Pythonパスが正常か検証
- [x] **デプロイ実行**:
    - [x] `devros deploy` コマンド実装
    - [x] ローカルデプロイ機能
    - [x] リモートデプロイ機能 (rsync over SSH)
    - [x] *Test (E2E)*: ローカルデプロイ後のディレクトリでノードが起動できるか確認
